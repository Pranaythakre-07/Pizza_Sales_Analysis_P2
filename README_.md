...
The database `pizzahut` consists of four interrelated tables:

```
orders ──────────── order_details ──────────── pizzas ──────────── pizza_types
(order_id PK)       (order_details_id PK)      (pizza_id PK)       (pizza_type_id PK)
(order_date)        (order_id FK)              (pizza_type_id FK)  (name)
(order_time)        (pizza_id FK)              (size)              (category)
...
2. **Set Up the Database**: Run the SQL scripts in `database_setup.sql` to create and populate all four tables.
3. **Run the Queries**: Execute the queries in `analysis_queries.sql` to reproduce all findings.
4. **Explore and Modify**: Adapt the queries to answer additional business questions or extend the analysis.

---

## Author

This project is part of my data analyst portfolio, showcasing SQL skills across database design, data exploration, and business analysis.
